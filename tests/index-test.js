...
const EXECUTABLE_PATH = path.join(__dirname, '..', 'bin', 'get-github-tag');
  ...
  QUnit.module('binary', function() {
    ...
    QUnit.test(
      ...
      async function(assert) {
        fs.writeFileSync(
          'package.json',
          JSON.stringify({ dependencies: { 'ember-data': '^1.10.0' } }),
          { encoding: 'utf8' }
        );

        let results = await execa(EXECUTABLE_PATH, [
          'emberjs',
          'data',
          'canary',
          '--write',
    ...
    QUnit.test(
      ...
      async function(assert) {
        fs.writeFileSync(
          'package.json',
          JSON.stringify({ devDependencies: { 'ember-data': '^1.10.0' } }),
          { encoding: 'utf8' }
        );
    ...
    QUnit.test('updates local package.json when -w is passed (devDependencies)', async function(
      assert
    ) {
      fs.writeFileSync('package.json', JSON.stringify({ devDependencies: { data: '^1.10.0' } }), {
        encoding: 'utf8',
    ...
    QUnit.test('preserves line ending when updating package.json', async function(assert) {
      fs.writeFileSync(
        'package.json',
        JSON.stringify({ dependencies: { data: '^3.10.0' } }, null, 2) + '\n',
        { encoding: 'utf8' }
      );
